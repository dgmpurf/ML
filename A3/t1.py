...
def read_data():
    ...
    with open('ratings-imdb.csv', encoding="utf8") as csv_file:
        csv_reader = csv.reader(csv_file)
        line_num = 0
        for row in csv_reader:
            if line_num > 0: # skip header
...
def rank_train_3 (classifier):
    return classifier.fit(X,y)

def mean_reciprocal_rank_test(dataset, classifier):
    # movieIds = []
    # for example in dataset:
    #     movieId1 = example[1]
    #     movieId2 = example[2]
    ...
    #     if movieId2 not in movieIds:
    #         movieIds.append(movieId2)
    # ranked_movieIds = label_ranking_average_precision_score(movieIds, classifier)
    # print(ranked_movieIds)
    movieIds = []
    for example in dataset:
        movieId1 = example[1]
        movieId2 = example[2]
        ...
        if movieId2 not in movieIds:
            movieIds.append(movieId2)
    movieIds = (np.asarray(r).nonzero()[0] for r in movieIds)
    return np.mean([1. / (r[0] + 1) if r.size else 0. for r in movieIds])
...
def main():
    ...
    # classifier = MultinomialNB()
    # classifier = rank_train_2(classifier)
    # mean_reciprocal_rank_test(dataset, classifier)
